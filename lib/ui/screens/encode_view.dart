...
class _EncodeViewState extends State<EncodeView> {
  ...
  void _generateRandomKey() {
    ...
    Random rnd = Random();
    String newKey = String.fromCharCodes(
  ...
  void _showSuccessDialog(String path) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          "The document has been sealed and archived to Gallery.",
          ...
          ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              await Share.shareXFiles([
                XFile(path),
              ], text: 'StenoCrypt Intelligence Report');
            },
            icon: const Icon(Icons.send, size: 16),
            label: const Text(
              "DISPATCH",
              style: TextStyle(fontFamily: 'Typewriter'),
            ),
          ),
          TextButton(
            onPressed: () {
  ...
  Future<void> _processEncoding() async {
    if (_selectedImage == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Missing Intel: Image, Payload, or Key required!"),
        ),
      );
      return;
    }
    ...
    try {
      String encryptedText = AESCipher.encryptText(
        _messageController.text,
        _passwordController.text,
      );
      final imageBytes = await _selectedImage!.readAsBytes();
      final Uint8List resultBytes = LSBEngine.encodeMessage(
        imageBytes,
        encryptedText,
      );
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  const Text(
                    "KEY SECURITY: ",
                    style: TextStyle(fontFamily: 'Typewriter', fontSize: 10),
                  ),
                  Text(
                    _strengthLabel,
                    style: TextStyle(
